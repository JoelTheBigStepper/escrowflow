...
contracts/
  ...
  EscrowFlowFactory.sol    factory that deploys minimal-proxy clones + keeps a registry
app/
...
scripts/
  ...
  deploy.mjs    deploys EscrowFlowFactory to Monad Testnet
```
...
`EscrowFlowFactory` yourself and point the frontend at your instance.
...
`contracts/EscrowFlowFactory.sol` into [Remix](https://remix.ethereum.org), compile with
Solidity 0.8.20+, connect MetaMask to Monad Testnet (chain ID `10143`, RPC
`https://testnet-rpc.monad.xyz`), and deploy `EscrowFlowFactory` (no constructor args — it
...
- `EscrowFlowFactory.createAgreement(type, title, deadlineDays, participants, provider)`
